...
使用 `Git` 的大体流程就是，在本地的工作区进行开发和修改，提交时先把修改放入暂存区，再把暂存区的所有修改提交到本地的版本库，最后把本地的版本库推送至类似于 `Github` 这样的远程仓库当中。

* 工作区（Working Directory）：电脑里能看到的文件目录，或者说就是物理上的项目代码文件夹。

* 暂存区（Stage）：

* 版本库（Repository）：每一个工作区都有一个隐藏的目录 `.git`，这个就是该项目仓库的版本库。
...
把一个代码仓库从远程下载到本地，实际上是把本地的 `master` 分支和远程的 `master` 分支对应起来了。这个命令的使用场景是：先在 `Github` 上创建好了一个远程仓库，然后直接把远程库下载到本地成为本地库，本地并不需要使用 `git init` 创建仓库。

##### git remote add origin git@github.com:<Github用户名>/<Github仓库名>.git
将本地的仓库和 `Github` 的远程仓库进行关联。这个命令的使用场景是：分别在 `Github` 上和本地都创建了项目仓库，
