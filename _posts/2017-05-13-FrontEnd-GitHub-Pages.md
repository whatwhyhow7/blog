...
进入刚刚新建的项目设置页面，找到 `GitHub Pages` 设置项，这时的显示应该为：
...
尽管 GitHub Pages 为我们提供了几个还算不错的默认的页面主题，但是对于做前端开发的同学肯定不会满意。这时我们就会考虑能不能在默认的主题基础上修改或者套用其他更好看的页面主题呢？答案自然是肯定的。
...
那么想使用自定义的主题，除了自己按照 `Jekyll` 规定的项目结构和配置进行编写以外，其实网上有很多别人写好的 [Jekyll模板](http://jekyllthemes.org/)，我们只需要选择其中自己喜欢的模板主题，将其对应的项目文件放入我们的项目目录中，稍加修改就更换主题成功了。